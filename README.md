...
### 🧞 11. Mermaid 逻辑绘图 (Mermaid Logic)
> **代码即图形的极简生产力** — 业务流程、系统架构，随口一说，即刻成图。
- **多模式支持**: 完美支持 Flowchart, Sequence, Gantt, Class, State, ER 等全量 Mermaid 图表。
- **智能排版系统**: 自动计算节点包络空间，内置 Auto-fit 引擎，确保任意比例图形均能完美呈现。
- **语义建模**: 集成 AI 助手，支持通过自然语言描述直接生成复杂的架构与流程逻辑。
...
- 🧞 **[Mermaid 逻辑绘图手册](docs/USER_MANUAL_MERMAID.md)**
...
### v3.5.3-ildr (Latest) - 2026-03-31
- **矩阵图 (Matrix) 渲染算法深度校准**: 
  - 修复 T 型矩阵水平偏移问题，通过“轴标签宽度补偿”算法实现重心精准居中。
  - 优化 X/L/C 型矩阵的几何测算，解决长文本在边界处的极端裁剪问题。
- **关联图 (Relation) UI 语义对齐**: 
  - 明确“症结 (Rect)”与“因素 (Ellipse)”的视觉区分逻辑，移除冗余标题栏。

### v3.5.2-ildr - 2026-03-31
- **关联图 (Relation) 深度重构**: 
  - 取消 `root` 节点自动生成逻辑，Title 仅作为导出元数据使用。
  - 支持多症结并存（出度为0），并应用 **矩形 (Rect)** 样式，普通因素应用 **椭圆 (Ellipse)** 样式。
  - 实施自引用拦截（禁止 A -> A）并增强 `fitView` 视口自适应，优化 Fruchterman 布局向心力。
- **基础图表 (Basic v2.0) 推理升级**: 
  - 将 `render_basic` 提升为 AI 首选推理引擎，增强对多层同心圆、多轴联动（Y/Y2/Y3）的逻辑支持。
  - 核心 Trigger 优化，确保典型统计图表（饼/柱/线）优先命中原生高性能组件。
- **矩阵图 (Matrix) 响应式布局**: 
  - 实现 Y 型矩阵长文本动态旋转及多维度避让算法，中心画幅利用率提升至极致。
  - 标准化 Top-Down 视角，统一 4px 全局自适应边距。
- **协议同步**: 全面更新 MCP 专家规则与 DSL 手册，确保多组件间的灵魂规则与渲染逻辑 100% 对齐。

### v3.5.1-ildr (Latest) Riverside,
- 修复 VChart 组件“导出 PNG”按钮无响应的问题。
- 标准化 15 种图表组件的 `BaseDiagramRef` 导出接口（`exportPNG`, `exportPDF`）。
- 增强导出协议，支持全局透明度与分辨率倍率配置。

### v3.5.0-ildr - 2026-03-26
- **ILDR 2.0 智能推理引擎**: 引入“双阶段推理”架构。第一阶段进行意图发现，第二阶段动态注入专项协议，彻底解决多引擎间的语法干扰。
- **分类分级治理**: 实现了具备“类型觉知”的输出红线。针对 VChart (Title/Spec)、Mermaid (Native) 和 IQS Native (Multi-paradigm) 自动切换最适约束，严禁语法冒穿。
- **协议库 100% 对齐**: 完成 18 种 VChart 组件与 13 种 IQS 原生工具的官方示例同步，确保 AI 推理获取高保原“种子数据”。
- **暴力纠错拦截**: MCP 服务端引入 JSON 物理拦截机制，对误传 JSON 的行为实施“秒级纠错”，显著降低 Token 消耗与重试率。
- **复杂组件协议修复**: 修正了桑基图 (Sankey)、旭日图 (Sunburst) 等递归组件的层级定义（Nodes + Children），确保大数据集渲染稳定性。

### v3.3.0 - 2026-03-24
- **ILDR (Internal Logic Driven Rendering) 架构实施**: 彻底改写了无头渲染流水线，从“视口截图”演进为“组件内部逻辑导出”，解决了 ECharts/VChart 在 Puppeteer 下的长久布局塌陷问题。
- **100% 尺寸对齐**: 图表组件现在严格遵循 MCP 请求的真实尺寸（Width/Height），并支持在导出瞬间进行动态 Resize。
- **3X 高清渲染增强**: 全量组件对齐 3X 分辨率导出协议，确保 3600x2400 及以上画幅的极致清晰度。
- **SVG 与 Mermaid 导出加固**: 优化了矢量合成逻辑，支持自适应边距裁剪。

### v3.2.0 - 2026-03-24
- **VChart 深度集成**: 支持高级配色、精细字号控制及 VChart 智能编辑器。
- **MCP 专家服务极大化**: 提供 50+ 专项工具矩阵，覆盖 Core/Mermaid/VChart 子类型。
- **SSE 渲染增强**: 实施 3X 高清采样及“单模 URL”优化，提升极致跨平台兼容性。
- **工程化优化**: 修复 Vite/MCP 端口冲突，回归单根目录依赖管理架构。

### v3.1.5 - 2026-03-20
- **Mermaid 指令集扩展**: 引入网络包结构图、架构图、象限图、XY图等高级逻辑组件。
- **UI 细节调优**: AI 服务状态显示优化，统一所有组件的导出参数接口。

### v3.1.0 - 2026-03-15
- **SPC 控制图增强**: 内置 Nelson Rules 判异引擎，支持 I-MR / X-bar-R 多模型渲染。
- **基础图表扩展**: 支持双轴联动、图例交互管控及多维度数据集映射。

### v3.0.0 - 2026-03-01
- **PDPC 风险决策分析**: 引入递归分组、多态连线及补偿措施建模能力。
- **矢线图 (CPM)**: 实施关键路径计算模型，支持实/虚活动拓扑布局。
- **多向矩阵系统**: 实现 Y/X/T/L/C 五大工业标准矩阵。

### v2.5.0 - 2026-02-15
- **视觉风格演进**: 实施 Industrial Light OS 规范，支持全局深色模式/明亮模式切换。
- **基础设施**: 迁移至 Tailwind CSS 本地化构建逻辑，移除外部 CDN 依赖。

### v2.0.0 - 2026-01-20
- **部署现代化**: 支持 Docker 容器化运行，引入 `docker-entrypoint` 运行时环境注入技术。
- **AI 核心升级**: 全面适配 Gemini Pro 1.5 视觉与文本推理。

### v1.0.0 - 2026-01-05
- **项目初始化**: 实现鱼骨图、排列图、直方图、散点图、亲和图等核心 QC 工具库。
- **DSL 引擎**: 确立“描述即图表”的文本驱动渲染原则。

© 2026 Smart QC Studio | Industrial Logic Factory 🏆🏁 - v3.5.2-ildr
