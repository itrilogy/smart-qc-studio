...
*   **核心度分析**：箭头指出最多的节点（出度高）通常是**根源问题 (Source)**；箭头指向最多的节点（入度高）通常是**核心症结 (Sink)**。
*   **环路识别**：如果在关联图中发现闭环（A->B->C->A），说明系统内部存在恶性循环，必须从环路中能量最低的一环进行切断。
*   **适用于复杂因果**：相比鱼骨图的线性结构，关联图更适合分析那些“牵一发而动全身”的交织矛盾。

---
*Smart QC Studio - 关联分析研究组*
