...
*   **视觉简约原则**：避免在同一个柱状图中展示超过 7 个分类，以免造成视觉拥挤；超过 5 个系列的折线图应考虑按特征拆分或使用堆叠模式。
*   **双轴的使用**：当比较销售额（千万级）与增长率（百分比级）时，必须使用副轴 (Y2)，否则增长率曲线将被压平为一条直线。
*   **饼图的局限**：严禁使用饼图展示分类超过 5 个且差异不明显的数据，此时应转而使用排列图 (Pareto)。
