...
*   **关键路径 (Critical Path)**：关键路径是整个项目的生命线。任何关键路径上的延期，都会直接导致项目交付时间的整体推迟。
*   **虚活动的妙用**：时长为 0 的虚活动（Dummy Activity）不仅是绘图需要，更是为了描述不消耗时间但存在逻辑先后约束的必要手段。
*   **资源平衡优化**：若发现关键路径过于漫长，应考虑将非关键路径（有时差的路径）上的资源向关键路径倾斜。

---
*Smart QC Studio - 矢线图工程组*
