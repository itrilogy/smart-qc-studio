...
*   **均值与波动的区分**：均值图（X-bar）的变动通常意味着某个特定因素的突变（如更换了刀具或换了批次）；而极差图（R/S）的变动通常意味着整体过程稳定性的下降（如设备老化或环境温湿度剧烈波动）。
*   **界限的本质区别**：严禁混淆 **规格限 (USL/LSL)** 与 **控制限 (UCL/LCL)**。控制限是由制程本身的统计特性（3σ）决定的，反映了“过程能做成什么样”；规格限是客户要求的限制，反映了“产品必须做成什么样”。
*   **判异优先序**：Rule 1（超出限值）是必须停机排查的核心指标；Rule 2、3 等趋势性判异则更多用于预防性维护（在产生不合格品之前进行干预）。
