...
2. **工业级数据预处理算法**：
   - **智能权重排序**：内置稳定的排序算法，能处理包括长字符串、特殊符号及高精度浮点数在内的多种原始数据，严格遵循 quality control 规范。
...
*   **二八定律的应用**：排列图并非强制要求 A 类因素必须占 80%，而是提醒管理者将资源集中在“最左侧”的 2-3 个核心问题上。
*   **要因初步筛选**：在 QC 成果报告中，排列图通常紧随鱼骨图之后。鱼骨图负责“找全”原因，排列图负责“锁定”要因。
*   **改善后的对比**：在效果检查阶段，应绘制改善后的排列图与改善前进行“对等对比”，观察柱体高度的削减情况和主要症结的位移。
