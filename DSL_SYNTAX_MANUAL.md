...
**分析场景**：分析多因素交叉影响、互为因果的网状关系。适用于原因交织、难以用层级工具描述的场景。

### 7.1 DSL 语法
- `Title: [文本]`：定义图形名称（主要用于导出文件命名，不显示在绘图区）。
- `Node: [ID], [Label]`：定义节点因素。
- `Rel: [Source] -> [Target]`：定义因果连线。**严禁自引用 (A -> A)**。
- `Layout: Directional | Centralized | Free`：布局模式。

### 7.2 核心逻辑
- **主要症结 (Root/Symptom)**：出度为 0 的节点，代表最终问题点。采用**矩形**样式以示强调。系统支持多个症结并存。
- **原因因素 (Factors)**：包括末端因素（入度为 0）和中间环节。均采用**椭圆**样式。
...
- **注意**：绘图区的图表标题已默认隐藏以腾出最大画布空间。DSL 中的 `Title:` 指令目前主要用于导出 PNG/PDF 时的文件命名，不再显示在实时绘图区。

### 8.2 类型解析
- **Y型**：采用标准化的 3D 俯视透视图 (Top-Down)，不再支持手动旋转视角，以确保视觉表现的一致性与稳定性。
...
- **自然语言驱动**：AI 会根据 `config.json` 协议将口语描述语义化。
